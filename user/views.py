...
def profile(request):
    ...
    success_complaints = Complaint.objects.filter(user=request.user, status='Success')
    pending_complaints = Complaint.objects.filter(user=request.user, status='Processed')
    bounty=len(success_complaints)*100
    content={
        "complaints":complaints,
        'success_complaints': success_complaints,
