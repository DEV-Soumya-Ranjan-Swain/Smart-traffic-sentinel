...
while cap.isOpened():
    ...
    if current_time - prev_time >= processing_interval:
        ...
        for box_person, score_person in zip(np.squeeze(boxes_motorbike), np.squeeze(scores_motorbike)):
            if (category_index_motorbike[int(classes_motorbike[0][0])]['name'] == 'person' and not np.all(box_person == 0) and score_person>=0.3) or category_index_motorbike[int(classes_motorbike[0][0])]['name'] == 'motorbike' and not np.all(box_person == 0) and scores_motorbike>=0.3:
                ...
                for box_helmet, score_helmet in zip(np.squeeze(boxes_helmet), np.squeeze(scores_helmet)):
                    ...
                    if not np.all(box_helmet == 0) and category_index_helmet[int(classes_helmet[0][0])]['name'] == 'with_helmet' and score_helmet <= 0.1 and score_person>=0.7  :
                ...
                # Draw a red bounding box if no helmet is detected inside the person box
                    if without_helmet_detected:
                        ...
                        cropped=image[int(ymin_person*imh):int(imh) ,int(xmin_person*imw)-100:int(xmax_person*imw) ]
